...
public class ExpresssoinValidateUtil {
    ...
    public static boolean checkLoginPwd(EditText edit) {
        String temp = edit.getText().toString().trim();
        pattern = Pattern.compile("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,20}$");
        macher = pattern.matcher(temp);
//		if (temp.length() < 6 || temp.length() > 20) {
//			T.ss("登录密码为6-20位字母加数字");
//			return false;
//		}
        return macher.find();
    }

    public static boolean checkPayPwd(EditText edit) {
        String temp = edit.getText().toString().trim();
