package com.hyf.tdlibrary.camera;
 ...
 * 这个运行在fragment中
 */
public class CameraIntentHelper2 {
	...
	public CameraIntentHelper2(Fragment fragment, CameraIntentHelperCallback cameraIntentHelperCallback) {
		mFragment = fragment;
		mCameraIntentHelperCallback = cameraIntentHelperCallback;
	}

    /**
     * Call to save the helpers instance state in mFragment's savedInstanceState.
     */
...
//				String sdkVersion = android.os.Build.VERSION.RELEASE.toLowerCase(Locale.ENGLISH);
				...
				if (setPreDefinedCameraUri) {
					...
					preDefinedCameraUri = mFragment.getActivity().getContentResolver().insert(
												MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
												values);
					intent.putExtra(MediaStore.EXTRA_OUTPUT, preDefinedCameraUri);
				}				
				mFragment.startActivityForResult(intent, CAPTURE_IMAGE_ACTIVITY_REQUEST_CODE);
	...
	private void onCameraIntentResult(int requestCode, int resultCode, Intent intent) {
		if (resultCode == Activity.RESULT_OK) {
			...
			try {
				...
				myCursor = mFragment.getActivity().getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
														largeFileProjection, 
														null, null, 
														largeFileSort);
	...
	private Uri getFileUriFromContentUri(Uri cameraPicUri) {
		...
		try {
			if (cameraPicUri != null
				...
				cursor = mFragment.getActivity().getContentResolver().query(cameraPicUri, proj, null, null, null);
				cursor.moveToFirst();
				// This will actually give you the file path location of the image.
